...
The product is "Claude, narrowly specialized in choosing AI models". There is no fixed answer template and no recommendation card: the LLM composes the entire answer as Markdown, streamed token by token, shaped by what the user actually asked. It builds its own tables when a side-by-side helps, chooses its own structure, and leads with reasoning.

What keeps this grounded is the FACTS block: a deterministic, per-turn digest of the candidate models carrying every number the LLM may use. Task-relevant benchmark scores by name (several per model, never one unexplained number), API prices and usage-based monthly estimates, context windows, and for local models the computed setup (quantization options with memory footprints from the quant_factors table and fit flags against the user's hardware, tok/s estimate by hardware tier with MoE models rated at active-parameter speed, install command, serving-stack guidance). The prompt requires every number to come from FACTS or from NOTES with its source, requires named benchmarks to be explained in a clause, and forbids recommending models absent from FACTS.

Answers stream: the recommend node pushes tokens through a contextvar channel that the web layer bridges to SSE `token` events (contextvars keep concurrent sessions isolated); the frontend re-renders the growing Markdown on each frame with a small safe renderer (all text lands via textContent, so model output cannot inject markup). Grounding runs after generation: any catalog model named outside the candidate set triggers a visible appended correction, logged, never silent.
